...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i = 0; i < wordLength ; i ++) {
			...
			boolean currCharIsVowel = vowels.indexOf(currChar) != -1;
			if ( currCharIsVowel && !lastCharIsVowel) {
				
				count++;
			}
			lastCharIsVowel = currCharIsVowel;
		}
		if (word.endsWith("e") && count > 1 && vowels.indexOf(word.charAt(wordLength - 2)) == -1) {//segue should count as two. e ue will count to 2, but should not minus 1 
			count -= 1;
		}
	    return count;
	}
	...
	public double getFleschScore()
	{
		...
		double numWords = (float)(getNumWords());
		double fleschScore = 206.835 - 1.015*numWords / getNumSentences() - 84.6 * getNumSyllables() / numWords;
	    return fleschScore;
	}
